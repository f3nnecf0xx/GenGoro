...
def getvideopage(domain,fcname):
 ...
 while True:
        ...
        for i in range(len(videoinfo["data"]["video_pages"]["list"])):
            ...
            videotype = str(videoinfo["data"]["video_pages"]["list"][i]["active_video_filename"]["video_filename_type"]["value"])#"vod_origin":
            if freeflg=="True" or freeper!="[]" or videotype=="vod_origin":
...
def start(url):
 ...
 time.sleep(0.5)
 if fanclubname!=0:
   if os.path.isdir(f"./{domain}/{fanclubname}")==True:
      file= os.listdir(f"./{domain}/{fanclubname}")
   else:
       file = []
 else:
   if os.path.isdir(f"./{domain}")==True:
      file= os.listdir(f"./{domain}")
   else:
       file = []
 for i in tqdm(range(len(videoidinfo[0]))):
     if fanclubname!=0:
         if not f"{videoidinfo[0][i]}.m3u8" in file:
